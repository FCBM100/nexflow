...
const steps = [
  {
    num: "01",
    title: "تحليل المشروع",
    desc: "فهم متطلباتك وأهدافك التجارية لوضع خطة عمل مخصصة.",
  },
  {
    num: "02",
    title: "تصميم الحل",
    desc: "رسم هيكلة النظام وتجربة المستخدم بما يناسب علامتك التجارية.",
  },
  {
    num: "03",
    title: "التطوير والبرمجة",
    desc: "بناء الموقع أو البوت أو نظام الأتمتة بأحدث التقنيات.",
  },
  {
    num: "04",
    title: "الاختبار والتحسين",
    desc: "اختبار شامل للأداء والتجربة وإجراء التحسينات اللازمة.",
  },
  {
    num: "05",
    title: "الإطلاق والمتابعة",
    desc: "نشر المشروع وتقديم دعم فني ومتابعة دورية لضمان الاستقرار.",
  },
];
...
export default function ProcessTimeline() {
  ...
  const animate = useCallback(async () => {
    ...
    // Traveling glow dot on connector (RTL: right→left)
            ...
            {steps.map((s, i) => (
              <div key={i} className="step-node flex flex-col items-center text-center group">
                <div className="relative z-10 w-16 h-16 rounded-full bg-surface border border-primary/20 flex items-center justify-center mb-4 group-hover:border-primary/50 transition-colors">
                  <span className="text-lg font-bold text-primary font-latin">
                    {s.num}
                  </span>
                </div>
            ...
            {steps.map((s, i) => (
              <div key={i} className="step-node flex items-start gap-4 group">
                <div className="relative z-10 w-12 h-12 rounded-full bg-surface border border-primary/20 flex items-center justify-center shrink-0">
                  <span className="text-base font-bold text-primary font-latin">
                    {s.num}
                  </span>
                </div>
                <div className="pt-2">
